...
static int fts_ctpm_get_app_i_file_ver(void)
{
    ...
    return CTPM_FW[APP_FILE_VER_MAPPING];
		...
		return CTPM_FW_ARA[APP_FILE_VER_MAPPING];
}
...
static int fts_ctpm_get_vendor_id(struct i2c_client * client)
{
    ...
    if (fts_updatefun_curr.app_valid == false)
    {
        ...
        if ((vendorid[1] != FTS_VENDOR_1_ID) && (vendorid[1] != FTS_VENDOR_2_ID) && (vendorid[1] != 0x3b) && (vendorid[1] != 0x03) && (uc_tp_fm_ver != 0xef))
        {
		...
		if (uc_tp_fm_ver == 0xef)
			printk("[fts][touch][UPGRADE] : firmware version error(0xef), force update firmware!\n");
...
static int fts_ctpm_fw_upgrade_with_app_i_file(struct i2c_client *client)
{
    ...
    i_ret = fts_ctpm_fw_upgrade_use_buf(client, CTPM_FW, fw_len);
		...
		i_ret = fts_ctpm_fw_upgrade_use_buf(client, CTPM_FW_ARA, fw_len);
...
static int fts_ctpm_fw_upgrade_with_app_bin_file(struct i2c_client *client, char *firmware_name)
{
    ...
    if ((pbt_buf[0x10C] != FTS_VENDOR_1_ID) && (pbt_buf[0x10C] != FTS_VENDOR_2_ID) && (pbt_buf[0x10C] != 0x3b) && (pbt_buf[0x10C] != 0x03))
    {
