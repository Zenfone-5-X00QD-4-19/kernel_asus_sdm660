...
unsigned char CTPM_FW_ARA[] =
{
#include FTS_UPGRADE_FW_APP_ARA
};
...
u8 fts_ctpm_check_fw_status(struct i2c_client *client)
{
    ...
    for (i=0; i<5; i++)
    {
        ...
        if ((uc_chip_id == chip_types.chip_idh) &&
            ((uc_tp_vendor_id == FTS_VENDOR_1_ID) || (uc_tp_vendor_id == FTS_VENDOR_2_ID) || (uc_tp_vendor_id == 0x3b) || (uc_tp_vendor_id == 0x03)))
        {
...
int fts_ctpm_fw_upgrade(struct i2c_client *client)
{
    ...
    if (i_ret < 0)
    {
        pr_err("[fts][touch][UPGRADE]: app.bin upgrade failed!!\n");
        return i_ret;
    }
    return 0;
}
...
int fts_ctpm_fw_upgrade(struct i2c_client *client)
{
    ...
    if (i_ret < 0)
    {
        pr_err("[fts][touch][UPGRADE]: app.i upgrade failed!!");
        return i_ret;
    }
    return 0;
}
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
    ...
    pr_err("[fts][touch] start auto upgrade ! \n");

    /* check need upgrade or not */
    bUpgradeFlag = fts_ctpm_check_need_upgrade(client);
    pr_err("[fts][touch][UPGRADE]: bUpgradeFlag = 0x%x \n", bUpgradeFlag);
    ...
    if (bUpgradeFlag)
    {
        do
        {
            ...
            if (i_ret == 0) /* upgrade success */
            {
                ...
                fts_i2c_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
                pr_err("[fts][touch][UPGRADE]: upgrade to new version 0x%x\n", uc_tp_fm_ver);
            }
            else /* upgrade fail */
            {
                fts_updatefun_curr.app_valid = false;
                ...
                pr_err("[fts][touch] [UPGRADE]: upgrade fail, reset now!!\n ");
    ...
    pr_err("[fts][touch]auto upgrade fw end \n");
    return i_ret;
}
#endif
...
static void fts_ctpm_update_work_func(struct work_struct *work)
{
    ...
    pr_err("[fts][touch] [UPGRADE]:FTS enter upgrade!!\n");
    ...
    if (i_ret < 0)
    {
        ...
        pr_err("[fts][touch][UPGRADE]: upgrade lcd cfg fail!!\n");
    }
#endif
...
err_workqueue_init:
    pr_err("[fts][touch] create_singlethread_workqueue failed\n");
    return -1;
}
...
void fts_ctpm_upgrade_init(void)
{
    ...
    if ( retval != 0 )
    {
        pr_err("[fts][touch][UPGRADE]fts_ctpm_workqueue_init failed! \n");
    }
    else
