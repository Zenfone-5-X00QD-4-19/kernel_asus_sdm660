...
{		
	//	struct fts_ts_data *data = dev_get_drvdata(dev);
		...
		if (fts_gesture_resume(fts_wq_data->client) == 0)
		{
			...
			disable_irq_nosync(fts_wq_data->client->irq);
			fts_reset_proc(300);
			if (!g_asus_lcd_power_off)
			{
				printk("[FTS][touch] read TP chip id\n");
                             ...
                             struct fts_ts_platform_data *pdata)
{
    ...
    if ((!strcmp(name, "focaltech,display-coords")) || (!strcmp(name, "focaltech,display-coords-ara")))
    {
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    rc = fts_get_dt_coords(dev, "focaltech,display-coords", pdata);
		...
		rc = fts_get_dt_coords(dev, "focaltech,display-coords-ara", pdata);
    if (rc)
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("[FTS][touch]g_asus_lcdID = %d\n", g_asus_lcdID);
    /* 1. Get Platform data */
