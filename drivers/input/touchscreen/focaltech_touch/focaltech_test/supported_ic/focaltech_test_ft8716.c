...
static unsigned char chCBData[TX_NUM_MAX * RX_NUM_MAX]  = {0};
...
static void Save_Test_Data(int iData[TX_NUM_MAX][RX_NUM_MAX],char *test_num, int RowArrayIndex, int ColArrayIndex,unsigned char Row, unsigned char Col, unsigned char ItemCount);
...
unsigned char FT8716U_TestItem_KEYShortTest(bool* bTestResult);
...
boolean FT8716_StartTest(void)
{
    ...
    for (iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++)
    {
        ...
        if (CODE_FT8716U_KEY_SHORT_TEST == g_stTestItem[ucDevice][iItemCount].ItemCode)
        {
            ReCode = FT8716U_TestItem_KEYShortTest(&bTempResult);
            if (ERROR_CODE_OK != ReCode || (!bTempResult))
            {
                bTestResult = false;
                g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
...
unsigned char FT8716_TestItem_RawDataTest(bool * bTestResult)
{
    ...
    //////////////////////////////Save Test Data
    Save_Test_Data(m_RawData,"RawData Test", 0, 0, g_stSCapConfEx.ChannelXNum+1, g_stSCapConfEx.ChannelYNum, 1);
    //----------------------------------------------------------Return Result
...
unsigned char FT8716_TestItem_CbTest(bool* bTestResult)
{
    ...
    //////////////////////////////Save Test Data
    Save_Test_Data(m_CBData,"CB Test", 0, 0, g_stSCapConfEx.ChannelXNum+1, g_stSCapConfEx.ChannelYNum, 1);
...
unsigned char FT8716_TestItem_ShortCircuitTest(bool* bTestResult)
{
    ...
    unsigned char iTxNum = 0, iRxNum = 0;
    ...
    if(g_ScreenSetParam.iSelectedIC == IC_FT8716) {
          iAllAdcDataNum = iTxNum * iRxNum + g_stSCapConfEx.KeyNumTotal;
    } else if (g_ScreenSetParam.iSelectedIC == IC_FT8716U 
                  || g_ScreenSetParam.iSelectedIC == IC_FT8613
                        || g_ScreenSetParam.iSelectedIC == IC_FT8716F) {
         iAllAdcDataNum = iTxNum * iRxNum;               
    }
    ...
    //////////////////////////////Save Test Data
    Save_Test_Data(shortRes,"Short Circuit Test", 0, 0, g_stSCapConfEx.ChannelXNum+1, g_stSCapConfEx.ChannelYNum, 1);
...
/************************************************************************
* Name: FT8716U_TestItem_KEYShortTest
...
unsigned char FT8716U_TestItem_KEYShortTest(bool* bTestResult)
{
	...
	unsigned char cKeyRes = -1;
	unsigned char iTxNum = 0, iRxNum = 0;
	short MaxVal = 0,MinVal = 0;	
	int i = 0;
	unsigned char cTestRes = -1;
	short TempVal;
	unsigned char bClbResult = 0;
	...
	//Get register 0x31 and save
	ReCode = ReadReg( 0x31, &cKey );
	ReCode = WriteReg( 0x31,  g_stCfg_FT8716_BasicThreshold.KEYShortTest_K1_Value);

	//Read the 0X0E register, if is 0, then set the success of K1
	for( i = 0; i < 6; ++i)
	{
		ReCode = ReadReg( 0x0E, &cKeyRes );
	...
	ReCode = WriteReg( 0x2E, 1);

	//check test is finished or not
	for( i = 0; i < 20; ++i)
	{
		
		ReCode =  ReadReg( 0x2F, &cTestRes);
	...
	//Read key CB, Compare with Max
	memset( chCBData, 0, sizeof(chCBData) );
	ReCode = GetTxRxCB( iTxNum*iRxNum, g_stSCapConfEx.KeyNumTotal*2, chCBData);
	if( ERROR_CODE_OK != ReCode )
	{
		bTempResult = false;
		FTS_TEST_DBG("\r\n\r\n//=========  CB test Failed!");
	...
	for( i = 0; i < (IsKeyAutoFit() ? g_stSCapConfEx.KeyNum : g_stSCapConfEx.KeyNumTotal)*2; i += 2)
	{
		if(0 == i)	 MinVal = MaxVal = (short)((chCBData[i] & 0x01 )<<8) + chCBData[i + 1];
		else
		{
		       TempVal = (short)((chCBData[i] & 0x01 )<<8) + chCBData[i + 1];
			if(TempVal > MaxVal)			MaxVal = TempVal;
			else if(TempVal < MinVal)		MinVal = TempVal;
		}
		
		FTS_TEST_DBG ("%d   ", (short)((chCBData[i] & 0x01 )<<8) + chCBData[i + 1]);		
		if(((short)((chCBData[i] & 0x01 )<<8) + chCBData[i + 1]) > g_stCfg_FT8716_BasicThreshold.KEYShortTest_CB_Max)
		{
			bTempResult = false;
		}	
	}
	...
	if( !bTempResult )
	{
		FTS_TEST_DBG("\r\n//========= Out of Threshold in KEY Short Test: \r\n");
		for(i = 0; i < (IsKeyAutoFit() ? g_stSCapConfEx.KeyNum : g_stSCapConfEx.KeyNumTotal)*2; i += 2)
		{
			if((short)((chCBData[i] & 0x01 )<<8) + chCBData[i + 1] > g_stCfg_FT8716_BasicThreshold.KEYShortTest_CB_Max)
			{
				FTS_TEST_DBG("Point( 0, %-2d): %-9d  ", i/2, (short)((chCBData[i] & 0x01 )<<8) + chCBData[i + 1]);
			}
		}
	}
...
unsigned char FT8716_TestItem_OpenTest(bool* bTestResult)
{
    ...
    unsigned char source_mode = 0xff;
    ...
    if (g_ScreenSetParam.iSelectedIC == IC_FT8716U 
            || g_ScreenSetParam.iSelectedIC == IC_FT8613
            || g_ScreenSetParam.iSelectedIC == IC_FT8716F) {

          ReCode = ReadReg(0x21, &source_mode);
          if (ERROR_CODE_OK != ReCode)
          {
              btmpresult = false;
              FTS_TEST_DBG("\r\n//=========  Read Reg Failed!");
    ...
    Save_Test_Data(m_CBData,"Open Test", 0, 0,  g_stSCapConfEx.ChannelXNum, g_stSCapConfEx.ChannelYNum, 1);
    ...
    if (g_ScreenSetParam.iSelectedIC == IC_FT8716U 
          ...
          ReCode = WriteReg(0x21, source_mode);
          SysDelay( 10);                      
          if (ERROR_CODE_OK != ReCode)
          {
              btmpresult = false;
...
//  Key
    ...
    if ( g_stCfg_FT8716_BasicThreshold.CBUniformityTest_Check_MinMax )
    {
	 ...
	 Save_Test_Data(tmpiUniform,"CB Uniformity Test", 0, 0, 1, 1, 3);
    ...
    if (g_stCfg_FT8716_BasicThreshold.CBUniformityTest_Check_CHX)
    {
        ...
        Save_Test_Data(CHX_Linearity,"CB Uniformity Test",  0, 1, g_stSCapConfEx.ChannelXNum,  g_stSCapConfEx.ChannelYNum-1, 1);
    ...
    if (g_stCfg_FT8716_BasicThreshold.CBUniformityTest_Check_CHY)
    {
        ...
        Save_Test_Data(CHY_Linearity,"CB Uniformity Test",  1, 0, g_stSCapConfEx.ChannelXNum-1,  g_stSCapConfEx.ChannelYNum, 2);
...
// Key
    ...
    Save_Test_Data(LCD_Noise,"LCD Noise Test", 0, 0, g_stSCapConfEx.ChannelXNum + 1, g_stSCapConfEx.ChannelYNum, 1 );
...
unsigned char FT8716_TestItem_RawDataUniformityTest(bool* bTestResult)
{
	...
	if( g_stCfg_FT8716_BasicThreshold.RawDataUniformityTest_Check_CHX )
	{
		FTS_TEST_DBG("\r\n=========Check Tx Linearity \r\n" );	
		 ...
		 Save_Test_Data(TxLinearity,"RawData Uniformity Test", 0, 1,  g_stSCapConfEx.ChannelXNum, g_stSCapConfEx.ChannelYNum-1, 1);
	}
	...
	if( g_stCfg_FT8716_BasicThreshold.RawDataUniformityTest_Check_CHY)
	{
		FTS_TEST_DBG("\r\n=========Check Rx Linearity \r\n");	
		 ...
		 for (iRow = 1; iRow<g_stSCapConfEx.ChannelXNum; iRow++)
		  ...
		  Save_Test_Data(RxLinearity,"RawData Uniformity Test", 1, 0,  g_stSCapConfEx.ChannelXNum-1, g_stSCapConfEx.ChannelYNum, 2);
	}
