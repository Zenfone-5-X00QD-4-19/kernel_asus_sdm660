...
static int fts_test_entry(char *ini_file_name)
{
	...
	printk(KERN_ERR "[FTS] [TEST]%s. line: %d.  ini_file_name = %s \n",  __FUNCTION__, __LINE__, cfgname);
  //  FTS_TEST_DBG("ini_file_name = %s", cfgname);
    ...
    {
        printk("[FTS][touch] get testparam from ini failure");
        goto TEST_ERR;
    }
    ...
    {
		TP_TEST_RESULT = 2;
       printk("[FTS][touch]tp test pass \n");
    }

    else
    {
		TP_TEST_RESULT = 1;
        printk("[FTS][touch] tp test failure \n");
    }
    ...
    fts_test_save_test_data("TP_Raw_Data.csv", testdata, iTestDataLen);
    fts_test_save_test_data("TP_Raw_Data.txt", testdata,iTestDataLen);
...
static ssize_t fts_test_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    //FTS_TEST_FUNC_ENTER();
	printk(KERN_ERR "[FTS][touch][TEST]%s: Enter(%d)\n", __func__, __LINE__);
    memset(fwname, 0, sizeof(fwname));
    sprintf(fwname, "%s", buf);
    fwname[count-1] = '\0';
  //  FTS_TEST_DBG("fwname:%s.", fwname);
	printk(KERN_ERR "[FTS] [TEST]%s. line: %d.  fwname:%s. \n",  __FUNCTION__, __LINE__, fwname);
