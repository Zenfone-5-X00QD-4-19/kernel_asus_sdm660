...
static void gf_kernel_key_input(struct gf_dev *gf_dev, struct gf_key *gf_key)
{
	...
	pr_info("[GDX_FP]: received key event[%d], key=%d, value=%d\n", key_input, gf_key->key, gf_key->value);
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GF_IOC_INIT:
		...
		break;
	case GF_IOC_EXIT:
		printk("%s GF_IOC_EXIT\n", __func__);
		break;
	case GF_IOC_DISABLE_IRQ:
		printk("%s GF_IOC_DISABEL_IRQ\n", __func__);
		gf_disable_irq(gf_dev);
		break;
	case GF_IOC_ENABLE_IRQ:
		printk("%s GF_IOC_ENABLE_IRQ\n", __func__);
		gf_enable_irq(gf_dev);
		break;
	case GF_IOC_RESET:
		printk("%s GF_IOC_RESET. \n", __func__);
		gf_hw_reset(gf_dev, 3);
		break;
	case GF_IOC_INPUT_KEY_EVENT:
		if (copy_from_user(&gf_key, (struct gf_key *)arg, sizeof(struct gf_key))) {
			printk("Failed to copy input key event from user to kernel\n");
			retval = -EFAULT;
			break;
		}
	...
	case GF_IOC_NAV_EVENT:
		printk("%s GF_IOC_NAV_EVENT\n", __func__);
		if (copy_from_user(&nav_event, (gf_nav_event_t *)arg, sizeof(gf_nav_event_t))) {
			printk("Failed to copy nav event from user to kernel\n");
			retval = -EFAULT;
			break;
		}
...
//		printk("Doesn't support control clock\n");
	...
	case GF_IOC_ENTER_SLEEP_MODE:
		printk("%s GF_IOC_ENTER_SLEEP_MODE\n", __func__);
		break;
	case GF_IOC_GET_FW_INFO:
		printk("%s GF_IOC_GET_FW_INFO\n", __func__);
		break;
	case GF_IOC_REMOVE:
		printk("%s GF_IOC_REMOVE\n", __func__);
		break;
	case GF_IOC_CHIP_INFO:
		printk("%s GF_IOC_CHIP_INFO\n", __func__);
...
#ifdef DELAY_POWER_ON
#define POWER_ON_DELAY_MS 20000 //20sec
struct gf_dev *gf_dev_ptr = 0;
static struct delayed_work __delay_power_on;
void delay_power_on(struct work_struct *work)
{

	printk("[GDX_FP] delay power on (%dms): set FP_3V3_EN +++\n",POWER_ON_DELAY_MS);
	if (gf_power_on(gf_dev_ptr) < 0) {
...
#endif
{
...
#ifdef DELAY_POWER_ON
	gf_dev_ptr = gf_dev;
	schedule_delayed_work(&__delay_power_on, msecs_to_jiffies(POWER_ON_DELAY_MS));
#else
	printk("[GDX_FP] gf_power_on: set FP_3V3_EN +++\n");
	...
	pr_info("[GDX_FP] version V%d.%d.%02d\n", VER_MAJOR, VER_MINOR, PATCH_LEVEL);
...
static int __init gf_init(void)
{
...
#ifdef DELAY_POWER_ON
	INIT_DELAYED_WORK(&__delay_power_on, delay_power_on);
#endif
	...
	printk("[GDX_FP] platform driver register \n");
	status = platform_driver_register(&gf_driver);
#elif defined(USE_SPI_BUS)
	printk("[GDX_FP] spi driver register \n");
	status = spi_register_driver(&gf_driver);
#endif
	if (status < 0) {
		class_destroy(gf_class);
		unregister_chrdev(SPIDEV_MAJOR, gf_driver.driver.name);
		printk("[GDX_FP] Failed to register SPI driver.\n");
	}

#ifdef GF_NETLINK_ENABLE
	netlink_init();
#endif
	printk("[GDX_FP] status = 0x%x\n", status);
	return 0;
}
module_init(gf_init);
