	...
	if (status == 0) {
		if (status == 0) {
			...
			if (gf_dev->users == 1) {
				...
				pr_info("gf_parse_dts  status= %d\n",
					status);
				if (status)
					goto err_parse_dt;

				status = irq_setup(gf_dev);
				pr_info("irq_setup  status= %d\n",
					status);
				if (status)
					goto err_irq;
			}
...
#define GOODIX_READ_MESSAGE "GOODIX 5228\n"

static ssize_t gf_read( struct file *filp,char __user *buf, size_t
   len, loff_t *off)
{
	return simple_read_from_buffer(buf, len, off,
			GOODIX_READ_MESSAGE, strlen(GOODIX_READ_MESSAGE) + 1);
    return 0;
}


static ssize_t gf_write(struct file *filp, const char __user *userbuf,
