...
bool g_ois_hw_version = FALSE; //ASUS_BSP Lucien +++: Forcing set ois to center mode with ER1 hw for Ara project
...
#define PAIR 3
	...
	uint32_t reg_addr[PAIR*2] = {	0x0258,0x025c,//Gyro
									0x05e0,0x060c,//Acc
									0x01b0,0x01b4//Hall
						        };
	int j;
...
//ASUS_BSP Lucien +++: Forcing set ois to center mode with ER1 hw for Ara project
int32_t ois_read_hw_version(bool ver)
{
	int rc = 0;
	mutex_lock(ois_ctrl->ois_mutex);
	g_ois_hw_version = ver;
	pr_err("%s: Is OIS ER1 hw stage? %s!\n", __func__, g_ois_hw_version? "Yes":"No");
	mutex_unlock(ois_ctrl->ois_mutex);
