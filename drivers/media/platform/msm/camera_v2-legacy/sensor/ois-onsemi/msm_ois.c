	...
	uint16_t size, struct reg_settings_ois_t *settings)
{
	...
	/*ASUS_BSP --- bill_chen "Implement ois"*/
       ...
       //ASUS_BSP Lucien +++: Check ois status before writing i2c bus
	uint8_t ois_status;
	int cnt;
	//ASUS_BSP Lucien ---: Check ois status before writing i2c bus
	CDBG("Enter\n");
	...
	for (i = 0; i < size; i++) {
		...
		case MSM_OIS_WRITE: {
			...
			case MSM_CAMERA_I2C_BYTE_DATA:
				...
				rc = onsemi_write_byte(o_ctrl,settings[i].reg_addr,(uint16_t)settings[i].reg_data);
				break;
			case MSM_CAMERA_I2C_WORD_DATA:
				pr_info("msm_ois_write_settings [%d], write 0x%X to reg 0x%X\n",
				    i,settings[i].reg_data,settings[i].reg_addr
					);
				rc = onsemi_write_word(o_ctrl,settings[i].reg_addr,(uint16_t)settings[i].reg_data);
				break;
			case MSM_CAMERA_I2C_DWORD_DATA:
				//ASUS_BSP Lucien +++: Check ois status before writing i2c bus
				while(ois_status)
				{
					ois_status = F40_RdStatus(o_ctrl,1);
					if(ois_status != 0)
					{
						delay_ms(10);
						cnt++;
				...
				//ASUS_BSP Lucien ---: Check ois status before writing i2c bus
				pr_info("msm_ois_write_settings [%d], write 0x%X to reg 0x%X waiting cnt %d\n",
				    i,settings[i].reg_data,settings[i].reg_addr, cnt
					);
				rc = onsemi_write_dword(o_ctrl,settings[i].reg_addr,settings[i].reg_data);
				break;
		...
		case MSM_OIS_READ: {
			...
			case MSM_CAMERA_I2C_READ_MODE_DATA:
				...
				{
					if(g_ois_hw_version == TRUE && ( g_ASUS_hwID == ZE620KL_ER || g_ASUS_hwID == ZE620KL_ER2))
					{
						g_ois_mode = 0;
						pr_info("ER1 sku set ois to center mode\n");//ASUS_BSP Lucien +++: Forcing set ois to center mode with ER1 hw for Ara project
					}else{
			...
			case MSM_CAMERA_I2C_READ_CALIBRATION_DATA:
				...
				break;
			case MSM_CAMERA_I2C_BYTE_DATA:
				rc = onsemi_read_byte(o_ctrl,settings[i].reg_addr,(uint16_t*)&read_num);
				...
				break;
			case MSM_CAMERA_I2C_WORD_DATA:
				rc = onsemi_read_word(o_ctrl,settings[i].reg_addr,(uint16_t*)&read_num);
				pr_info("msm_ois_write_setting [%d], read 0x%x from reg 0x%x\n",i, read_num, settings[i].reg_addr);
				if(settings[i].reg_data != read_num)
				{
					pr_err("read_num 0x%x not match expected value = 0x%x\n", read_num,settings[i].reg_data);
				...
				break;
			case MSM_CAMERA_I2C_DWORD_DATA:
				rc = onsemi_read_dword(o_ctrl,settings[i].reg_addr,&read_num);
				pr_info("msm_ois_write_setting [%d], read 0x%x from reg 0x%x\n",i, read_num, settings[i].reg_addr);
				if(settings[i].reg_data != read_num)
				{
					pr_err("read_num 0x%x not match expected value = 0x%x\n", read_num,settings[i].reg_data);
		...
		case MSM_OIS_POLL: {
			...
			case MSM_CAMERA_I2C_BYTE_DATA:
				...
				rc = onsemi_poll_byte(o_ctrl,settings[i].reg_addr,settings[i].reg_data,settings[i].delay);
			...
			case MSM_CAMERA_I2C_WORD_DATA:
				...
				rc = onsemi_poll_word(o_ctrl,settings[i].reg_addr,settings[i].reg_data,settings[i].delay);
	...
	void __user *argp)
{
	...
	case CFG_OIS_I2C_WRITE_MODE: {
		...
		{
	    	conf_array.delay = 0;
		//pr_info("%s: Forcing to center mode with ER1 hw\n", __func__);
		}
		//ASUS_BSP Lucien ---: Forcing set ois to center mode with ER1 hw for Ara project
		rc = onsemi_switch_mode(msm_ois_t_pointer,conf_array.delay);
		if(rc == 0)
		{
...
static const struct of_device_id msm_ois_dt_match[] = {
	{.compatible = "qcom,lc898123f40", .data = NULL},
	{}
};
...
//ASUS_BSP Lucien +++: Auto update OIS firmware at device booting
...
uint32_t current_fw, uint8_t vcm)
{
	...
	if( (current_fw >>24) == 0x03 && g_ASUS_hwID == ZE620KL_ER )
	{
		update_fw = true;
		vcm = 0x01;
		pr_info("%s: ER1 sku always update fw and overwrite LiteOn vcm to ver.1\n", __func__);
	}
	//ASUS_BSP Lucien ---: For recovery wrong ois fw issue
#endif
	pr_info("%s: update_fw = %s\n", __func__, update_fw ? "TRUE": "FALSE");
	...
	if( update_fw )
	{
		...
		pr_info("OIS firmware is the lastest. No need to update firmware.\n");
...
static int32_t msm_ois_check_id(struct msm_ois_ctrl_t *o_ctrl) {
	...
	// Read CVER (Low level)+++
	rc = onsemi_write_dword(o_ctrl, 0xC000,0x00D00100);
	if(rc < 0) {
        pr_err("%s%d: ois write i2c fail!\n", __func__, __LINE__);
		goto OIS_CHECK_FAIL;
       }

       rc = onsemi_read_dword(o_ctrl, 0xD000,&chipid);
       if(rc < 0) {
        pr_err("%s%d: ois read i2c fail!\n", __func__, __LINE__);
		goto OIS_CHECK_FAIL;
       }
	// Read CVER (Low level)---
	...
	o_ctrl->fw_ver = fw_version;  //ASUS_BSP Lucien +++: Using service to auto update ois fw
