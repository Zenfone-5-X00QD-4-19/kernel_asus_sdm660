...
//ASUS_BSP  +++ Ryan "distinguish ov module version"
static bool isFrontR1A;
static bool isRearR1A;
//ASUS_BSP  --- Ryan "distinguish ov module version"
...
{	
	...
	printk("%s : E , sensor_id = %d\n",__func__, sensor_id);
         if(sensor_id == SENSOR_ID_IMX362 || sensor_id == SENSOR_ID_IMX363){
...
{
 	int   i, j, package_num,k;
 	int32_t rc = 0;
	u16 bank_value[MODULE_OTP_SIZE], temp;
	u16 Early_sample[10]; //ASUS_BSP +++ For ov 8856 early sample distinguish ov version
	u16 FrontVer, RearVer;//ASUS_BSP  +++ Ryan "distinguish ov module version"
	struct msm_sensor_ctrl_t  *s_ctrl = g_sensor_ctrl[camNum];
	...
	for (k =0;k < 10 ;k++){
		sensor_read_reg(s_ctrl,0x7000+k,&Early_sample[k]);
		//printk("ryan_k camera id = %d Early_sample[%d] = 0x%02x",camNum,k,Early_sample[k]);
		}
	
	if (camNum == 1){
		sensor_read_reg(s_ctrl,0x700f,&FrontVer);
			if (FrontVer == 0x02){
				isFrontR1A = 0;//R1B
				printk("ryan_k cameraid = %d r1b",camNum);
			}else if (FrontVer == 0x00 && 
				Early_sample[0] == 0x00 &&
				Early_sample[1] == 0x88 &&
				Early_sample[2] == 0x56 &&
				Early_sample[3] == 0x0a &&
				...
				Early_sample[8] == 0x08 &&
				Early_sample[9] == 0x01){
				isFrontR1A = 0;//R1B early sample
				printk("ryan_k cameraid = %d early sample r1b",camNum);
			}else{
				isFrontR1A = 1;//R1A
				printk("ryan_k cameraid = %d r1a",camNum);
				}
	}else if (camNum == 2){
		sensor_read_reg(s_ctrl,0x700f,&RearVer);
			if (RearVer == 0x02){
				isRearR1A = 0;//R1B
				printk("ryan_k cameraid = %d r1b",camNum);
			}else if (RearVer == 0x00 && 
				Early_sample[0] == 0x00 &&
				Early_sample[1] == 0x88 &&
				Early_sample[2] == 0x56 &&
				Early_sample[3] == 0x0a &&
...
{	
	int i, rc, j;
	static int cnt = 0;
	bool ois_er1_hw;//ASUS_BSP Lucien +++
	/*
	for( i = 0; i < OTP_DATA_LEN_WORD; i++)
	{
	...
	*/
	...
	if(cnt < 1)
	{
		//ASUS_BSP Lucien +++: Forcing set ois to center mode with ER1 hw for Ara project
		for(j = 0; j < CHECK_OIS_DATA_LEN ; j++)
		{
			if( g_rear_eeprom_mapdata.mapdata[0x422+j] == 0xff) cnt++;
		}

		if(cnt > DISABLE_OIS_LEN)
			ois_er1_hw = TRUE;
		else
			ois_er1_hw = FALSE;

		rc = ois_read_hw_version(ois_er1_hw);
		if(rc < 0) pr_err("%s: ois_read_hw_version failed\n", __func__);
		//ASUS_BSP Lucien ---: Forcing set ois to center mode with ER1 hw for Ara project
	}
	return 0;
}
...
void create_proc_otp_thermal_file(struct msm_camera_sensor_slave_info *slave_info)
{
	   ...
	   //read otp file & thermal node
   	   if(sensorID == SENSOR_ID_IMX363 || sensorID==SENSOR_ID_IMX298 || sensorID==SENSOR_ID_IMX362 || sensorID==SENSOR_ID_S5K3M3 ||sensorID==SENSOR_ID_IMX214){
   		create_dump_eeprom_file(cameraID, sensorID);
   		create_thermal_file(cameraID, sensorID);
		create_camera_res_proc_file(); //ASUS_BSP Lucien +++: Add node for reading camera resolutions
   	   }

	   if(sensorID==SENSOR_ID_IMX363 || sensorID==SENSOR_ID_IMX362 || sensorID==SENSOR_ID_S5K3M3 || sensorID==SENSOR_ID_IMX214 || sensorID == SENSOR_ID_OV8856){
...
int rear_thermal_read(struct seq_file *buf, void *v)
{
	...
	uint16_t val=0;
	  ...
	  if(g_sensor_id[CAMERA_0]==SENSOR_ID_IMX298 || g_sensor_id[CAMERA_0]==SENSOR_ID_IMX362 || g_sensor_id[CAMERA_0]==SENSOR_ID_IMX363){
...
void create_thermal_file(int camID, int sensorID)
  {
         if(camID == 0){
	  if(sensorID == SENSOR_ID_IMX298 || sensorID == SENSOR_ID_IMX362 ||sensorID == SENSOR_ID_IMX363){
	  	   //for thermal team=======================
...
#define ARCSOFTDATA_START 0x0422//0x0834
#define ARCSOFTDATA_END    0x0C21//0x1034
static int read_eeprom_arcsoft_dualCali(struct seq_file *buf, void *v)
{
	...
	if(g_rear_eeprom_mapdata.mapdata != NULL)
	{
		 for ( i = ARCSOFTDATA_START; i <( ARCSOFTDATA_END+1) && i < g_rear_eeprom_mapdata.num_data;  i++)
		 {
...
{
	   if(camID == 0){
		if(sensorID == SENSOR_ID_IMX362 || sensorID == SENSOR_ID_IMX363){
  ...
  int rear_1_module_proc_read(struct seq_file *buf, void *v)
{
	  ...
	  }else if(g_sensor_id[CAMERA_0]==SENSOR_ID_IMX363){
		  seq_printf(buf,"IMX363\n");
	  }
	  return 0;
}
  ...
  int front_module_proc_read(struct seq_file *buf, void *v)
{
  ...
  //ASUS_BSP  +++ Ryan "distinguish ov module version"
  int ov_cam_front_read(struct seq_file *buf, void *v)
  {
	  if(g_sensor_id[CAMERA_1]==SENSOR_ID_OV8856){
		if (isFrontR1A== 0)
			seq_printf(buf,"OV8856_R1B\n");
		else 
			seq_printf(buf,"OV8856_R1A\n");
	  }
		return 0;
  }
  int ov_cam_rear_read(struct seq_file *buf, void *v)
  {
	  if(g_sensor_id[CAMERA_2]==SENSOR_ID_OV8856){
	  	if (isRearR1A== 0)
			seq_printf(buf,"OV8856_R1B\n");
		else 
			seq_printf(buf,"OV8856_R1A\n");
  ...
  //ASUS_BSP Lucien +++: Add node for reading camera resolutions
  int camera_res_read(struct seq_file *buf, void *v)
  {
      char *rear1 = NULL;
	  char *rear2 = NULL;
	  char *front = NULL;

	  rear1 = kzalloc(sizeof(char)*5,GFP_KERNEL);
	  rear2 = kzalloc(sizeof(char)*5,GFP_KERNEL);
	  front = kzalloc(sizeof(char)*5,GFP_KERNEL);
	  if(g_sensor_id[CAMERA_0]==SENSOR_ID_IMX362)
		  sprintf(rear1,"12M");
	  else if(g_sensor_id[CAMERA_0]==SENSOR_ID_S5K3M3)
		  sprintf(rear1,"13M");
  ...
  int front_module_proc_open(struct inode *inode, struct  file *file)
{
  ...
  //ASUS_BSP  +++ Ryan "distinguish ov module version"
   int ov_cam_front_open(struct inode *inode, struct file *file)
{
	  return single_open(file, ov_cam_front_read, NULL);
}
	int ov_cam_rear_open(struct inode *inode, struct file *file)
{
	   return single_open(file, ov_cam_rear_read, NULL);
}
	//ASUS_BSP	--- Ryan "distinguish ov module version"

	//ASUS_BSP Lucien +++: Add node for reading camera resolutions
   int camera_res_open(struct inode *inode, struct file *file)
{
	  return single_open(file, camera_res_read, NULL);
}
	//ASUS_BSP Lucien ---: Add node for reading camera resolutions
  ...
  //ASUS_BSP  +++ Ryan "distinguish ov module version"
  const struct file_operations ov_cam_front_fops = {
	  .owner = THIS_MODULE,
	  .open = ov_cam_front_open,
	  .read = seq_read,
	  .llseek = seq_lseek,
	  .release = single_release,
  };
  const struct file_operations ov_cam_rear_fops = {
	.owner = THIS_MODULE,
	.open = ov_cam_rear_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
  ...
  //ASUS_BSP Lucien +++: Add node for reading camera resolutions
  const struct file_operations camera_res_fops = {
	  .owner = THIS_MODULE,
	  .open = camera_res_open,
	  .read = seq_read,
	  .llseek = seq_lseek,
	  .release = single_release,
  };
  ...
  void create_front_module_proc_file(void)
{
  ...
  void create_ov_cam_proc_file(int cameraID)
  {
	  if(cameraID == CAMERA_1){
			status_proc_file = proc_create(PROC_FILE_OV_FRONTCAM, 0666, NULL, &ov_cam_front_fops);
			 if(status_proc_file) {
				  CDBG("%s_1  sucessed!\n", __func__);
			  } else {
				  pr_err("%s_1 failed!\n", __func__);
			...
			status_proc_file = proc_create(PROC_FILE_OV_REARCAM, 0666, NULL, &ov_cam_rear_fops);
			 if(status_proc_file) {
				  CDBG("%s_2 sucessed!\n", __func__);
			  } else {
				  pr_err("%s_2 failed!\n", __func__);
	...
	//ASUS_BSP Lucien +++: Add node for reading camera resolutions
  void create_camera_res_proc_file(void)
  {
	status_proc_file = proc_create(PROC_FILE_CAMERA_RES, 0666, NULL, &camera_res_fops);
		if(status_proc_file) {
			CDBG("%s_1  sucessed!\n", __func__);
		} else {
			pr_err("%s_1 failed!\n", __func__);
...
//ASUS_BSP	+++ Ryan "distinguish ov module version"
 void remove_ov_cam_file(void)
 {
	extern struct proc_dir_entry proc_root;
    pr_info("remove_ov_cam_version_file\n");
	remove_proc_entry(PROC_FILE_OV_REARCAM, &proc_root);
    remove_proc_entry(PROC_FILE_OV_FRONTCAM, &proc_root);
 }
//ASUS_BSP  --- Ryan "distinguish ov module version"

//ASUS_BSP Lucien +++: Add node for reading camera resolutions
 void remove_camera_res_file(void)
 {
	extern struct proc_dir_entry proc_root;
    pr_info("remove_camera_res_file\n");
	remove_proc_entry(PROC_FILE_CAMERA_RES, &proc_root);
 }
//ASUS_BSP Lucien ---: Add node for reading camera resolutions
