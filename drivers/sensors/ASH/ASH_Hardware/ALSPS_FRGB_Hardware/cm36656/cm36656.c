...
#undef dbg
	...
	#define dbg(fmt, args...)
#endif
#define log(fmt, args...) pr_debug(KERN_INFO "[%s][%s]"fmt,MODULE_NAME,SENSOR_TYPE_NAME,##args)
...
static int cm36656_regulator_init(void){
	...
	reg = regulator_get(&g_i2c_client->dev,"vcc_ana");
    if (IS_ERR_OR_NULL(reg)) {
        ret = PTR_ERR(reg);
        err("Failed to get regulator vcc_ana %d\n", ret);
        return ret;
    }
    ret = regulator_set_voltage(reg, 3000000, 3600000);
    if (ret) {
        err("Failed to set voltage for vcc_ana reg %d\n", ret);
        return -1;
    }
    
    log("ldob3 regulator setting init");
    return ret;
}
...
static int cm36656_regulator_enable(void){
    ...
    if (ret < 0) {
        err("Failed to set load for vcc_ana reg %d\n", ret);
        return ret;
    }
    
    ret = regulator_enable(reg);
    if (ret) {
        err("Failed to enable vcc_ana reg %d\n", ret);
        return -1;
    }
    
    for(idx=0; idx<10; idx++){
        if(regulator_is_enabled(reg) > 0){
            dbg("ldob3 regulator is enabled(idx=%d)", idx);
            break;
        }
    }
    if(idx >= 10){
        err("ldob3 regulator is enabled fail(tryty count >= %d)", idx);
        return -1;
    }
    
    log("Update ldob3 to NPM_mode");
    return ret;
}
