...
#undef dbg
	...
	#define dbg(fmt, args...)
#endif
#define log(fmt, args...) pr_debug(KERN_INFO "[%s]"fmt,MODULE_NAME,##args)
#define err(fmt, args...) pr_debug(KERN_ERR "[%s]"fmt,MODULE_NAME,##args)
...
{	
	...
	switch(type){
		...
		case SAR_sensor:
			...
			sensor_dev = device_create(g_property_class, NULL, dev, NULL, "%s", "SAR_sensor");
			break;
		default:
			err("%s: Type ERROR.(%d)\n", __FUNCTION__, type);
