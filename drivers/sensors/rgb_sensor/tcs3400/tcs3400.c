...
#define RGB_DELAY_MULTIPLES_NUMERATOR 4
#define RGB_DELAY_MULTIPLES_DENOMINATOR 4
...
#define ASUS_RGB_SENSOR_IOCTL_AGAIN_SET           _IOW(ASUS_RGB_SENSOR_IOC_MAGIC, 5, int)	///< RGB sensor ioctl command - Set AGAIN value
...
#endif
	g_rgb_status.delay_time_ms = (256 - cm_lp_info->it_time) * 278 / 100 *RGB_DELAY_MULTIPLES_NUMERATOR/RGB_DELAY_MULTIPLES_DENOMINATOR;
	 /* g_rgb_status.delay_time_ms = (256 - cm_lp_info->it_time) * (2.78) * ( DELAY_MULTIPLES_NUMERATOR/DELAY_MULTIPLES_DENOMINATOR)  */
	RGB_DBG("%s: set delay time to %d ms, reg %d\n", __func__, g_rgb_status.delay_time_ms, cm_lp_info->it_time);	
...
static void rgbSensor_checkStatus(void)
{
	...
	if (g_rgb_status.need_wait) {//Eason: check after enable RGB sensor
...
static int rgbSensor_itSet_ms(int input_ms)
{
	...
	//if (itSet_count % IT_TIME_LOG_SAMPLE_RATE == 0) {
...
bool checkRgbcCycleCompleted(int Total_Test_Number)
{
 	int t_count;  
	u8 adc_status;
 	int ret;
	...
	for(t_count=0; t_count< Total_Test_Number; t_count++)
	{
	        ...
	        */
	        ...
	        if (ret < 0) {
		        RGB_DBG("%s,TCS3400_STATUS  I2C error", __func__ );
		        return -EIO;
	        }
...
bool is_it_time_passed(struct timeval start, struct timeval now){
	int l_it_time = (256 - cm_lp_info->it_time) *278/100 *RGB_DELAY_MULTIPLES_NUMERATOR/RGB_DELAY_MULTIPLES_DENOMINATOR * 1000;
	 /* l_it_time = (256 - cm_lp_info->it_time) * (2.78) *  (DELAY_MULTIPLES_NUMERATOR/DELAY_MULTIPLES_DENOMINATOR) * 1000 */
	...
	if(elapse_time > l_it_time) {
		RGB_DBG("%s: it  = %dus has passed, elapse time = %lldus!!\n", __func__, l_it_time, elapse_time);
		return true;
	}
	return false;
}
...
static int tcs3400_als_showrgbc_proc_read(struct seq_file *buf, void *v)
{
	...
	//Eason: delay integration time
	integration_time = ( 256 -cm_lp_info->it_time )*278/100;
	msleep(integration_time);
	RGB_DBG("%s: after msleep %d ms\n", __func__, integration_time);
	
	checkRgbcCycleCompleted(20);
    
	// Get initial RED light data
	ret = get_rgb_data(RGB_DATA_R, &adc_data[0], false);
	if (ret < 0) {
		return -EIO;
	}
	ret = get_rgb_data(RGB_DATA_G, &adc_data[1], false);
	if (ret < 0) {
		return -EIO;
	}
	ret = get_rgb_data(RGB_DATA_B, &adc_data[2], false);
	if (ret < 0) {
		return -EIO;
	}
	ret = get_rgb_data(RGB_DATA_W, &adc_data[3], false);
	if (ret < 0) {
		return -EIO;
	}
		...
		size_t len, loff_t *data)
{
	...
	if (val >= 0 && val <= 255) {
		...
		if (cm_lp_info != NULL && cm_lp_info->als_enable == 1) {
			RGB_DBG("%s: %d => %x\n", __func__, val, val << 4);
			_tcs3400_I2C_Mask_Write_Word(TCS3400_ADDR, TCS3400_CONF, CFG_IT_MASK, val);
		} else{
...
static long rgbSensor_miscIoctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case ASUS_RGB_SENSOR_IOCTL_DEBUG_MODE:
			...
			break;
		case ASUS_RGB_SENSOR_IOCTL_AGAIN_SET:
			if (cm_lp_info->als_enable != 1) {
				ret = -2;
				RGB_DBG_E("%s: als not enabled yet!\n", __func__);
				goto end;
			}
			ret = copy_from_user(&again_setting, (int __user*)arg, sizeof(again_setting));
			if (ret < 0) {
